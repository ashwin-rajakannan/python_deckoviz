...
const AllQueueCollection = () => {
  ...
  // Track which button is focused in the currently focused card
  const [focusedButton, setFocusedButton] = useState('display'); // 'heart' or 'display'
  ...
  const cardData = collectionQueue.map((collection, index) => {
  ...
  const filteredData = activeFilter === 'Favourite'
  ...
  const handleDisplayPress = useCallback((item) => {
    navigation.navigate('SpecificCollection', { currentCollection: item });
  }, [navigation]);

  const handleHeartPress = useCallback((item) => {
  ...
  useTVEventHandler(useCallback((evt) => {
    if (evt.eventType === 'right') {
      ...
      } else if (focusSection === 'cards' && focusedButton === 'display') {
        // Move to next card if on display button
        if (focusedCardIndex < filteredData.length - 1) {
    ...
    if (evt.eventType === 'left') {
      if (focusSection === 'filters' && focusedFilterIndex > 0) {
        ...
        // Move to previous card if on display button
    ...
    if (evt.eventType === 'down' && focusSection === 'filters') {
      ...
      setFocusedButton('display'); // Default to display button when coming from filters
    }
    ...
    // Toggle between heart and display buttons when up/down is pressed on a card
    if (focusSection === 'cards') {
      if (evt.eventType === 'up' && focusedButton === 'display') {
        setFocusedButton('heart');
      } else if (evt.eventType === 'down' && focusedButton === 'heart') {
        setFocusedButton('display');
      }
    }
    ...
    if (evt.eventType === 'select' && focusSection === 'cards') {
      ...
      if (focusedButton === 'display') {
       // handleDisplayPress(selected.fullData);
      } else if (focusedButton === 'heart') {
        handleHeartPress(selected);
      }
    }
  }, [
    focusSection,
  ...
  const renderItem = useCallback(({ item, index }) => (
    <View style={styles.cardSpacing}>
      <FavouriteCards
        ...
        // Pass focus state for buttons
        isHeartFocused={focusSection === 'cards' && index === focusedCardIndex && focusedButton === 'heart'}
        isDisplayFocused={focusSection === 'cards' && index === focusedCardIndex && focusedButton === 'display'}
      />
    </View>
  ), [
    focusSection,
  ...
  return (
    <ScrollView style={styles.background}>
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.title}>All Collection In Queue</Text>
      </View>
      ...
      <FlatList
        ...
        extraData={{ focusedCardIndex, favorites, focusSection, focusedButton }}
      />
    </ScrollView>
  );
};
...
export default AllQueueCollection;